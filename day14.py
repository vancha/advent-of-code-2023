...
class platform:
    ...
    #tilt the grid left, and then back to neutral
    def tilt_left(self):
        work_to_do = True
            ...
            for row_idx, row in enumerate(self.grid):
                for idx, spot in enumerate(''.join(row)):
                    ...
                    if idx > 0 and spot == "O" and row[idx - 1] == ".":
                        work_to_do = True
                        self.grid[row_idx] = row[0 : idx - 1] + row[idx] + "." + row[idx + 1 :]
                        
    #rotate the entire grid right
    def rotate_right(self):
        #i stole this line... it's pretty cool though
        self.grid = list(zip(*self.grid[::-1])) #[''.join(list(row)) for row in zip(*self.grid)]
